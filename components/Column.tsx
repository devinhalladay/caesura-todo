...
const Column = ({ date, tasks, userId, innerRef, handleOpenModal }) => {
  ...
  const progressBarWidth = Object.values(tasks).length
    ? (Object.values(tasks).filter((task) => task.completed === true).length /
  ...
  return (
    <div className="mr-4" ref={innerRef}>
      <p className="font-sans-serif text-2xl font-bold">{dayOfWeek}</p>
      <p className="text-sm text-gray-500 mb-2">
        {dayjs(date).format("MMMM DD")}
      </p>
      <div className="h-2 relative w-full bg-gray-200 rounded-full overflow-hidden mb-4">
        <span
          className="absolute top-0 left-0 h-full w-10 bg-green-500 transition-all"
